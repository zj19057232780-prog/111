...
def main():
    ...
    parser.add_argument('--noise-test', action='store_true', help='Test saved weights at configured SNRs; no training unless --train')
    parser.add_argument(
        '--model_path',
    ...
    if args.noise_test:
        args.test = True
    if not args.train and not args.test and not args.preprocess:
        mode = cfg.get('run_mode', 'train_test')
        args.train = mode in ('train', 'train_test')
        args.test = mode in ('test', 'train_test')
    print(f"Method: {cfg.get('method', 'configured_maml')}")
    print(f"Actions: train={args.train}, test={args.test}, "
          f"noise_test={bool(args.noise_test or cfg.get('noise_test', False))}")
    ...
    if cfg.get('method') == 'tl_wdcnn':
        print(f"Input: raw vibration [batch, 1, {cfg['window_size']}]")
    ...
    print(f'Task weighting: {cfg.get("task_weighting_mode", "none")}')
    ...
    if args.train or args.test:
        ...
        if args.test:
            ...
            if args.noise_test or cfg.get('noise_test', False):
                from cwru_noise_test import run_noise_test
                run_noise_test(learner, best_path, quick=quick)
            else:
                learner.test(
                    best_path,
